...
- **质量第一**：代码质量和系统安全不可妥协
- **中文交流**：统一使用中文回复、注释和文档
- **思考先行**：编码前必须深度分析和规划，若有疑问直接终止代码修改，直接向用户询问
- **工具优先**：优先使用验证过的最佳工具链
- **透明记录**：关键决策和变更必须可追溯
- **持续改进**：从每次执行中学习和优化
- **结果导向**：以目标达成为最终评判标准
...
2. 工具链 → 逐步实施代码与脚本变更
3. 验证测试 → 确保修改正确性，例如使用 `gh` 查看构建工作流运行是否正常

---

## 📌 Git 提交规范

**每一条、每一批提交都必须推送至远程仓库，不能仅保留在本地仓库。**

- 提交者：`Jyf0214 <169313142+Jyf0214@users.noreply.github.com>`
- 提交信息：中文，清晰描述变更内容
...
1. 任何时候都要追求最大并行 Agent 数
2. 一个 Agent 完成后立即启动新的替补，保持流水线满负荷
3. 如果某 Agent 执行缓慢，启动更多 Agent 协助同一或独立任务
4. 不等结果就发起下一批，批量启动要激进
5. 小任务可直接在主会话处理，但仍追求最大吞吐量
6. 每次启动前自问"还能再开吗？"——能就开
