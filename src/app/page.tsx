    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-4xl mx-auto px-4 pb-24">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {['projectManagement', 'cloudService'].map((key) => (
            <div key={key} className="border border-gray-200 rounded-xl p-6 hover:border-gray-300 hover:shadow-sm transition-all">
              <h3 className="font-semibold text-gray-900 mb-2">{t(key as 'projectManagement')}</h3>
              <p className="text-sm text-gray-500">{t((key === 'projectManagement' ? 'projectManagementDesc' : 'cloudServiceDesc') as 'projectManagementDesc')}</p>
            </div>
          ))}
        </div>
      </div>
