...
export const GET = withApiLogging('GET sync', async function GET(request: Request) {
  try {
    ...
    return errorResponse('获取同步状态失败', 'GET_STATUS_FAILED', 500);
  }
});
...
export const POST = withApiLogging('POST sync', async function POST(request: Request) {
  try {
    ...
    if (action === 'test') {
      ...
      return errorResponse('连接失败，请检查服务器地址、凭据和远程路径', 'CONNECTION_FAILED', 400);
    }

    if (action === 'pull') {
      const client = await createWebdavClient();
      if (!client) {
        return errorResponse('WebDAV 未配置', 'NOT_CONFIGURED', 400);
      }
      const config = await db.webdavConfig.findFirst();
      const localDir = process.env.SYNC_LOCAL_DIR ?? './sync';
      const count = await pullFromRemote(client, localDir, config?.remotePath ?? '/');
      return successResponse({ message: '已拉取 ' + String(count) + ' 个文件' });
    }
    ...
    if (action === 'push') {
      ...
      return successResponse({ message: '已推送 ' + String(pushedCount) + ' 个文件' });
    }

    if (action === 'start') {
      const ok = await startWatching();
      if (ok) {
        return successResponse({ message: '监听已启动' });
      }
      return errorResponse('配置未启用，请先保存并启用 WebDAV 配置', 'NOT_CONFIGURED', 400);
    }

    if (action === 'stop') {
      await stopWatching();
      return successResponse({ message: '监听已停止' });
    }

    return errorResponse('未知操作', 'UNKNOWN_ACTION', 400);
  } catch (err) {
    console.error('[Sync] 同步操作失败:', err);
    return errorResponse('同步操作失败', 'SYNC_FAILED', 500);
  }
});
...
export const DELETE = withApiLogging('DELETE sync', async function DELETE(request: Request) {
  try {
    ...
    return errorResponse('删除配置失败', 'DELETE_FAILED', 500);
  }
});
